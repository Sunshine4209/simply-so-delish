...
<div id="post-<?php the_ID(); ?>" <?php post_class('entry ' . $kale_post_class); ?>>
    ...
    <div class="entry-content">
        ...
        <?php if(get_the_title() != '') { ?>
            <h3 class="entry-title" style="min-height: 23px;margin-top: -15px;margin-bottom: 0;"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h3>
        <?php } else { ?>
            <h3 class="entry-title" style="min-height: 23px;margin-top: -15px;margin-bottom: 0;"><a href="<?php the_permalink(); ?>"><?php esc_html_e('Post ID: ', 'kale'); the_ID(); ?></a></h3>
        <?php } ?>
        
        <?php if($kale_blog_feed_meta_show == 1 && $kale_blog_feed_date_show == 1) { ?>
        <div class="entry-date date updated" style="margin-bottom: -5px;"><a href="<?php the_permalink(); ?>"><?php echo get_the_date(); ?></a></div>
        <?php } ?>
